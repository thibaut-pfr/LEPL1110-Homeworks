...
int tridiagonalize_band(double *A, int n, int k, double *d, double *e){
    ...
    for (int j = 0; j < n - 2; j++) {
        for (int i = MIN(j + k, n-1); i > j + 1; i--) {
            ...
            double temp_diag = get_safe(A,i-1,stop); 
            ...
            // Rotation coefficients
                ...
                for (int l = start; l < stop+1; l++) { 
                    if (l == stop){
                        temp_i = c * temp_diag + s * get_safe(A,i,l);
                        temp_j = - s * temp_diag + c * get_safe(A,i,l);
                    }
                    else if(l == start){
                        temp_i = c * get_safe(A,i-1,l) + s * temp_garbage;
                        temp_j = - s * get_safe(A,i-1,l) + c * temp_garbage;
                    }
                    else{
                ...
                for (int l = start; l < stop+1; l++) {
                    ...
                    if(l == stop){
                        temp_i = c * temp_garbage + s * get_safe(A,l,i);
                        temp_j = - s * temp_garbage + c * get_safe(A,l,i);
                    }
                    else if(l == start){
                        temp_i = c * get_safe(A,l,i-1) + s * temp_diag;
                        temp_j = - s * get_safe(A,l,i-1) + c * temp_diag;
                    }
                    else{
